#!/usr/bin/env python3
"""
Launch UNet Step 3 — UNet3DProto + prototype losses (5-fold CV)

Usage:
    python launch_unet_step3.py --bucket your-brats2020-data \
        --step2-weights-s3 s3://your-brats2020-data/unet-step2-outputs/.../output/model.tar.gz
    python launch_unet_step3.py --monitor unet-step3-20260510-160000
"""
...
def parse_args():
    ...
    p.add_argument('--output-prefix',      type=str, default='unet-step3-outputs')
    p.add_argument('--instance',           type=str, default='ml.g4dn.xlarge')
    p.add_argument('--volume-size',        type=int, default=100)
    p.add_argument('--max-runtime',        type=int, default=172800)  # 48h sufficient for 5×50
    ...
    p.add_argument('--step2-weights-s3',   type=str, default=None,
                   help='S3 URI of Step 2 model.tar.gz (from monitor output)')
    # Hyperparameters
...
def upload_code(bucket, job_name):
    code_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'unet_step3')
...
def launch(args):
    ...
    job_name  = args.job_name or f'unet-step3-{timestamp}'
    ...
    if args.step2_weights_s3:
        hp['step2-weights-s3'] = args.step2_weights_s3
    ...
    print(f'\nLoss weights: clst={args.clst_weight} sep={args.sep_weight} '
          f'div={args.div_weight}')
    print(f'\nFinal CV summary will show:')
    print(f'  Mean Dice : X.XXXX ± X.XXXX')
    print(f'  Proto ratios: NCR=X.XX ED=X.XX ET=X.XX  (NCR>1.0 = learned specificity)')
    if not args.step2_weights_s3:
        print('\n⚠  No --step2-weights-s3 provided — step3 will train from scratch')
