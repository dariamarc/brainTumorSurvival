#!/usr/bin/env python3
"""
Architecture 1 — UNet Step 3: UNet3DProto + prototype losses

Changes vs original step3:
  - base_channels=32
  - Data augmentation
  - 5-fold CV with fold-matched weight loading from Step 2
  - HD95 metric

Loss: dice + clst_weight*L_clst - sep_weight*L_sep + div_weight*L_div
"""
...
# ── Data generator (identical to step2) ──────────────────────────────────────
...
def make_train_step(model, opt, num_prototypes, protos_per_class,
                    clst_weight, sep_weight, div_weight, alpha_mdsc):
    """Return a tf.function-compiled training step closed over model & opt."""
    proto_dim = model.proto_dim

    @tf.function
    def step(imgs_t, masks_t, class_map_t):
        with tf.GradientTape() as tape:
            logits, distances = model.forward_train(imgs_t)
            l_ce   = weighted_ce_loss(masks_t, logits)
            l_mdsc = mdsc_loss(masks_t, logits)
            l_seg  = l_ce + alpha_mdsc * l_mdsc
            l_clst = proto_clustering_loss(distances, class_map_t,
                                           num_prototypes, protos_per_class, proto_dim)
            l_sep  = proto_separation_loss(distances, class_map_t,
                                           num_prototypes, protos_per_class, proto_dim)
            l_div  = proto_diversity_loss(model.prototype_vectors, protos_per_class)
            loss   = (l_seg
                      + clst_weight * l_clst
                      - sep_weight  * l_sep
                      + div_weight  * l_div)
        ...
        return loss, l_ce, l_mdsc, l_clst, l_sep, l_div
...
def proto_diversity_loss(prototype_vectors, protos_per_class):
    ...
    pvec = tf.reshape(prototype_vectors, [P, -1])
    pvec = tf.nn.l2_normalize(pvec, axis=1)
    total   = tf.constant(0.0)
    n_pairs = float(protos_per_class * (protos_per_class - 1))
    for c in range(3):
        i  = c * protos_per_class
        cp = pvec[i: i + protos_per_class]
        gram = tf.matmul(cp, cp, transpose_b=True)
...
# ── Weight loading from Step 2 ────────────────────────────────────────────────

def load_step2_weights(model, step2_dir, fold_idx):
    path = os.path.join(step2_dir, f'fold_{fold_idx}_best.weights.h5')
    if not os.path.exists(path):
        log.warning(f'Step 2 weights not found: {path}  — training from scratch')
        return
    try:
        model.load_weights(path)
        log.info(f'Loaded Step 2 weights (fold {fold_idx}): {path}')
    except Exception:
        model.load_weights(path, by_name=True, skip_mismatch=True)
        log.info(f'Loaded Step 2 weights by name (fold {fold_idx}): {path}')


def fetch_step2_weights(s3_uri):
    import boto3, tarfile
    s3_path     = s3_uri.replace('s3://', '')
    bucket, key = s3_path.split('/', 1)
    local_dir   = '/tmp/step2_weights'
    os.makedirs(local_dir, exist_ok=True)
    local_tar   = os.path.join(local_dir, 'model.tar.gz')
    log.info(f'Downloading Step 2 weights: s3://{bucket}/{key}')
...
def train_fold(fold_idx, trn_ids, val_ids, args, step2_dir):
    ...
    if step2_dir:
        load_step2_weights(model, step2_dir, fold_idx)

    opt       = keras.optimizers.Adam(learning_rate=args.lr)
    step_fn   = make_train_step(model, opt, model.num_prototypes, args.protos_per_class,
                                args.clst_weight, args.sep_weight, args.div_weight,
                                args.alpha_mdsc)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step in range(n_steps):
            ...
            class_map = downsample_mask(masks)

            loss, l_ce, l_mdsc, l_clst, l_sep, l_div = step_fn(imgs_t, masks_t, class_map)
            ep_losses.append({'total': float(loss), 'ce': float(l_ce),
                              'mdsc': float(l_mdsc), 'clst': float(l_clst),
                              'sep': float(l_sep), 'div': float(l_div)})
        ...
        m, ratios = run_validation(val_gen, args.protos_per_class, infer_fn,
                                   compute_hd95=False)
        ncr_r = np.mean([v for k, v in ratios.items() if 'ncr' in k])
        ed_r  = np.mean([v for k, v in ratios.items() if '_ed'  in k])
        et_r  = np.mean([v for k, v in ratios.items() if '_et'  in k])

        log.info(
            f'  Epoch {epoch:3d}/{args.epochs} | lr={cosine_lr:.2e} | '
            f'loss={avg["total"]:.4f} [ce={avg["ce"]:.4f} mdsc={avg["mdsc"]:.4f} '
            f'clst={avg["clst"]:.3f} sep={avg["sep"]:.3f} div={avg["div"]:.3f}] | '
...
def train(args):
    log.info('=' * 60)
    log.info('UNET STEP 3 — UNet3DProto + prototype losses  '
    ...
    step2_dir = None
    if args.step2_weights_s3:
        step2_dir = fetch_step2_weights(args.step2_weights_s3)
    elif args.step2_weights_dir and os.path.isdir(args.step2_weights_dir):
        step2_dir = args.step2_weights_dir
    ...
    for k, (trn_ids, val_ids) in enumerate(folds):
        result = train_fold(k, trn_ids, val_ids, args, step2_dir)
...
if __name__ == '__main__':
    ...
    p.add_argument('--step2-weights-s3',   type=str, default=None)
    p.add_argument('--step2-weights-dir',  type=str, default=None)
