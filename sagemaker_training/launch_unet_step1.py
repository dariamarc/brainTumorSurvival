#!/usr/bin/env python3
"""
Launch UNet Step 1 — Plain UNet3D baseline (base_channels=32, 5-fold CV, augmentation)

Usage:
    python launch_unet_step1.py --bucket your-brats2020-data
    python launch_unet_step1.py --monitor unet-step1-20260510-120000
"""
...
def parse_args():
    ...
    p.add_argument('--output-prefix', type=str, default='unet-step1-outputs')
...
def upload_code(bucket, job_name):
    code_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'unet_step1')
        ...
        if status != last:
            ...
            if status in ('Completed', 'Failed', 'Stopped'):
                ...
                if 'ModelArtifacts' in resp:
                    ...
                    print(f'Pass to step2 via --step1-weights-s3 : {uri}')
                break
        time.sleep(30)
...
def launch(args):
    ...
    job_name  = args.job_name or f'unet-step1-{timestamp}'
    ...
    print(f'  Mean HD95 : X.XX ± X.XX mm')
