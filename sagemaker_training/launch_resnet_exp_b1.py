#!/usr/bin/env python3
"""
Launch Experiment B1 — Full PrototypeSegNet3D, no prototype losses (5-fold CV)

Usage:
    python launch_resnet_exp_b1.py --bucket your-brats2020-data \
        --exp-a-weights-s3 s3://your-brats2020-data/resnet-exp-a-outputs/.../output/model.tar.gz
    python launch_resnet_exp_b1.py --monitor resnet-exp-b1-20260510-120000
"""
...
def parse_args():
    ...
    p.add_argument('--output-prefix',      type=str, default='resnet-exp-b1-outputs')
    p.add_argument('--instance',           type=str, default='ml.g4dn.xlarge')
    p.add_argument('--volume-size',        type=int, default=100)
    p.add_argument('--max-runtime',        type=int, default=172800)
    ...
    p.add_argument('--exp-a-weights-s3',   type=str, default=None,
                   help='S3 URI of Exp A model.tar.gz (from monitor output)')
    # Hyperparameters
...
def upload_code(bucket, job_name):
    code_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'resnet_exp_b1')
        ...
        if status != last:
            ...
            if status in ('Completed', 'Failed', 'Stopped'):
                ...
                if 'ModelArtifacts' in resp:
                    ...
                    print(f'Pass to Exp B2 via --exp-b1-weights-s3 : {uri}')
                break
        time.sleep(30)
...
def launch(args):
    ...
    job_name  = args.job_name or f'resnet-exp-b1-{timestamp}'
    ...
    hp = {
        ...
        'n-prototypes':   str(args.n_prototypes),
    ...
    if args.exp_a_weights_s3:
        hp['exp-a-weights-s3'] = args.exp_a_weights_s3
    ...
    if not args.exp_a_weights_s3:
        print('\n⚠  No --exp-a-weights-s3 — backbone will train from scratch')
    print(f'\nKey signal to watch: NCR ratio > 1.0 without explicit prototype losses')
