#!/usr/bin/env python3
"""
Launch Experiment B2 — Full PrototypeSegNet3D + prototype losses (5-fold CV)

Usage:
    python launch_resnet_exp_b2.py --bucket your-brats2020-data \
        --exp-b1-weights-s3 s3://your-brats2020-data/resnet-exp-b1-outputs/.../output/model.tar.gz
    python launch_resnet_exp_b2.py --monitor resnet-exp-b2-20260510-160000
"""
...
def parse_args():
    ...
    p.add_argument('--output-prefix',       type=str, default='resnet-exp-b2-outputs')
    ...
    p.add_argument('--exp-b1-weights-s3',   type=str, default=None,
                   help='S3 URI of Exp B1 model.tar.gz (from monitor output)')
    # Hyperparameters
...
def upload_code(bucket, job_name):
    code_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'resnet_exp_b2')
...
def launch(args):
    ...
    job_name  = args.job_name or f'resnet-exp-b2-{timestamp}'
    ...
    if args.exp_b1_weights_s3:
        hp['exp-b1-weights-s3'] = args.exp_b1_weights_s3
    ...
    print(f'Loss    : dice + {args.clst_weight}×clst − {args.sep_weight}×sep')
    if not args.exp_b1_weights_s3:
        print('\n⚠  No --exp-b1-weights-s3 — will train from scratch')
    print(f'\nPass criterion: NCR ratio > 1.0 across all folds')
